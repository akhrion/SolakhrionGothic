...
instance INFO_XARDAS_ALTRUNE_DEX(C_Info)
{
	npc = KDF_404_Xardas;
	condition = info_xardas_altrune_dex_condition;
	information = info_xardas_altrune_dex_info;
	important = 0;
	permanent = 0;
	description = "Можно ли перенести энергию горы не в УРИЗЕЛЬ, а..";
};


func int info_xardas_altrune_dex_condition()
{
	if(Npc_KnowsInfo(hero,Info_Xardas_FORMULA) && (Npc_GetTalentSkill(hero,NPC_TALENT_BOW) || Npc_GetTalentSkill(hero,NPC_TALENT_CROSSBOW)))
	{
		return TRUE;
	};
};

func void info_xardas_altrune_dex_info()
{
	AI_Output(other,self,"Info_Xardas_ALTRUNE_DEX_NULL_01");	//Можно ли перенести энергию горы не в УРИЗЕЛЬ, а..
	AI_Output(self,other,"Info_Xardas_ALTRUNE_DEX_NULL_02");	//Я тебя понял, я знаю один способ...
	AI_Output(self,other,"Info_Xardas_ALTRUNE_14_03");	//Но сначала восстанови силу меча. Потом я расскажу тебе об этом.
	Log_CreateTopic(GE_URIZELRUNE,LOG_NOTE);
	B_LogEntry(GE_URIZELRUNE,"Во время нашей последней встречи Ксардас упомянул о том, что силу УРИЗЕЛЯ необязательно хранить в мече. Но он расскажет мне об этом только после того, как я заряжу меч. Хмм..");
};
...
instance INFO_XARDAS_MAKEBOW(C_Info)
{
	...
	important = 0;
	permanent = 0;
	description = "Ты говорил, что я смогу использовать силу УРИЗЕЛЬЯ, без УРИЗЕЛЯ.";
};


func int info_xardas_makebow_condition()
{
	if(Npc_KnowsInfo(hero,info_xardas_altrune_dex) && Npc_KnowsInfo(hero,Info_Xardas_SWORDLOADED))
	{
		return TRUE;
	};
};
...
{
	AI_Output(other,self,"Info_Xardas_MAKEBOW_NULL_01");	//Ты говорил, что я смогу использовать силу УРИЗЕЛЬЯ, без УРИЗЕЛЯ.
	AI_Output(self,other,"Info_Xardas_MAKERUNE_14_02");	//Посмотри на него. Его самая важная деталь - синий мерцающий камень на клинке.
	AI_Output(self,other,"Info_Xardas_MAKERUNE_14_03");	//Он хранит в себе всю силу меча.
	AI_Output(self,other,"Info_Xardas_MAKERUNE_14_04");	//Если я выну этот камень, я смогу сделать из него руну, которая будет обладать всеми свойствами меча.
	AI_Output(other,self,"Info_Xardas_MAKERUNE_15_05");	//Магическую руну из УРИЗЕЛЯ?
	AI_Output(self,other,"Info_Xardas_MAKEBOW_NULL_06");	//Именно так. Сила этой руны может быть упрощена и при помощи своего оружия
	AI_Output(self,other,"Info_Xardas_MAKEBOW_NULL_07");	//Ты сможешь использовать мощь УРИЗЕЛЯ
};
...
instance INFO_XARDAS_MAKEBOWDOIT(C_Info)
{
	...
	important = 0;
	permanent = 1;
	description = "Хорошо, передай силу меча этому луку!";
};
...
{
	AI_Output(other,self,"Info_Xardas_MAKEBOWDOIT_NULL_01");	//Хорошо, передай силу меча этому луку!
	AI_Output(self,other,"Info_Xardas_MAKERUNEDOIT_14_03");	//Это будет необратимое изменение меча. Ты действительно хочешь, чтобы я вынул камень?
...
func void info_xardas_makebow_yes()
{
	...
	AI_RemoveWeapon(self);
	AI_UnequipWeapons(self);
	AI_Output(self,other,"Info_Xardas_MAKEBOWDOIT_NULL_05_03");	//Вот, возьми меч и УРИЗЕЛЬ в новом обличие!
	b_printtrademsg2("Получен УРИЗЕЛЬ и меч.");
	Npc_RemoveInvItem(hero,Mythrilklinge02);
	CreateInvItems(hero,itrw_urizelbow,1);
	CreateInvItems(hero,Mythrilklinge03,1);
	B_LogEntry(GE_URIZELRUNE,"Ксардас отделил магический камень от меча УРИЗЕЛЯ. Теперь вся сила меча перейдет в мой лук.");
};
