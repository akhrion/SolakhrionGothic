...
//        ||  Npc_IsInFightMode(hero,FMODE_FIST)
            ...
            PrintScreenSIS("spawned herb: ",SPWN_Cur_Swampherb_01,"",84,70,3);
            if(7)
            {
                if(!Hlp_Random(6))
                {
...
{
    if(C_BodyStateContains(hero,BS_STAND)){return;};
    if(!Wld_IsFPAvailable(hero,"SPWN_PLANT")){return;};
    
    if(Wld_IsFPAvailable(hero,"SPWN_PLANT_PSI")){
