...
const int NPC_FLAG_NFOCUS = 1 << 18;	//ѕри поддержке zNoFocusFlag.vdf делает Ќѕ— антаргет.
...
const int ITEM_RING = 1 << 21;		//¬ оригинальных скриптах индекс равен 1 << 11, однако при таком индексе движок игры не позвол€ет использовать больше 2х колец, следовательно индекс был изменен
const int ITEM_MISSION = 1 << 12;
const int ITEM_CLUB = 1 << 13;		//¬ оригинале эта константа отсутствует, однако оружие с данным индексом становитс€ в слот оружи€ ближнего бо€.
const int ITEM_SWD = 1 << 14;
const int ITEM_AXE = 1 << 15;
const int ITEM_2HD_SWD = 1 << 16;
const int ITEM_2HD_AXE = 1 << 17;
const int ITEM_2HD_STAFF = 1 << 18;	//¬ оригинале эта константа отсутствует, однако оружие с данным индексом становитс€ подсвеченным в инвентаре (бинд 1) что наталкивает на мысль использовать этот индекс как щит. ќднако оружие с этим индексом не добавл€ет защиту и не замен€ет само себ€, т.е. вы можете вз€ть 2.3.4.. щита - что есть чушь. ¬озможно этот индекс использовалс€ дл€ выбора аммуниции дл€ луков или просто недоделан - что странно. ¬озможно он не изучен до конца.
const int ITEM_BOW = 1 << 19;
const int ITEM_CROSSBOW = 1 << 20;
const int ITEM_MULTI = 1 << 21;
const int ITEM_AMULET = 1 << 22;
const int ITEM_UNTARGET = 1 << 23;	//√√ не сможет подобрать предмет, лежащий на земле (в оригинале этот флаг присваеваетс€ оружию вз€тому в руки)
...
const int ITEM_EQUIPED = 1 << 30;	//≈сли подобрать оружие с этим флагом, то оно будет сразу экипировано, однако пользоватьс€ им будет невозможно.
