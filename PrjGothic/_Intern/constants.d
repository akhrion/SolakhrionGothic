...
const int ITEM_RING = 1 << 21;		//В оригинальных скриптах индекс равен 1 << 11, однако при таком индексе движок игры не позволяет использовать больше 2х колец, следовательно индекс был изменен
const int ITEM_MISSION = 1 << 12;
const int ITEM_CLUB = 1 << 13;		//В оригинале эта константа отсутствует, однако оружие с данным индексом становится в слот оружия ближнего боя.
const int ITEM_SWD = 1 << 14;
const int ITEM_AXE = 1 << 15;
const int ITEM_2HD_SWD = 1 << 16;
const int ITEM_2HD_AXE = 1 << 17;
const int ITEM_2HD_STAFF = 1 << 18;	//В оригинале эта константа отсутствует, однако оружие с данным индексом становится подсвеченным в инвентаре (бинд 1) что наталкивает на мысль использовать этот индекс как щит. Однако оружие с этим индексом не добавляет защиту и не заменяет само себя, т.е. вы можете взять 2.3.4.. щита - что есть чушь. Возможно этот индекс использовался для выбора аммуниции для луков или просто недоделан - что странно. Возможно он не изучен до конца.
const int ITEM_BOW = 1 << 19;
const int ITEM_CROSSBOW = 1 << 20;
const int ITEM_MULTI = 1 << 21;
const int ITEM_AMULET = 1 << 22;
const int ITEM_UNTARGET = 1 << 23;	//ГГ не сможет подобрать предмет, лежащий на земле (в оригинале этот флаг присваевается оружию взятому в руки)
const int ITEM_DROPPED = 1 << 24;
const int ITEM_UNLIMITEDAMMO = 1 << 25;


const int ITEM_TORCH = 1 << 28;
const int ITEM_THROW = 1 << 29;
const int ITEM_EQUIPED = 1 << 30;	//Если подобрать оружие с этим флагом, то оно будет сразу экипировано, однако пользоваться им будет невозможно.
...
const int NPCPOISONEDMINIMALHP = 10;
const int POISONDAMAGE = 1;
const int NPCWOUNDEDMINIMALHP = 10;
const int WOUNDDAMAGE = 1;
const int DAM_CRITICAL_MULTIPLIER = 2;
const int HUNGRY_PROBABILITY = 5;
...
const int PROT_INDEX_BARRIER = 0;
const int PROT_INDEX_BLUNT = 1;
const int PROT_INDEX_EDGE = 2;
const int PROT_INDEX_FIRE = 3;
const int PROT_INDEX_FLY = 4;
const int PROT_INDEX_MAGIC = 5;
const int PROT_INDEX_POINT = 6;
const int PROT_INDEX_FALL = 7;
const int PROT_INDEX_MAX = 8;
const int PROT_IMMUN = 9999;
...
//Введено несколько префиксов в код
//C_ базовые изменения оперирующие константами, переменными и не требующие игрового процесса или инициализации игрой.
//F_ функциональные события выступающие в роли тригера - удар по лицу, поднятие объекта, бег или стояние, и т.д.
//G_ Игровая функция, которая отрабатывает тогда, когда обстоятельства соответствуют условиям. Так-же в роли тригера, может выступать движок. Или к примеру функция G_VisualChange_Fingal_RemoveFromPC вызываемая в цикле, но отработает она, только после изменения глобальной переменной, и некоторых условий.
