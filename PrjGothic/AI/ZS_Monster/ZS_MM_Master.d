...
func int ZS_MM_Attack_Loop()
{
	...
	if(Hlp_IsValidNpc(other) && !C_NpcIsDown(other))
	{
		...
		// )
		// {
		// 	Print("Œÿ»¡ ¿ ÒÍÂÎÂÚ ‡Ú‡ÍÛÂÚ Ï‡ÒÚÂ‡!");
			
		// 	if(Npc_GetNextTarget(self))
		// 	{
		// 		Print("ÂÒÚ¸ ÌÓ‚‡ˇ ˆÂÎ¸");
				
		// 		PrintDebugString(PD_MST_CHECK,"...ÂÒÚ¸ ÌÓ‚‡ˇ ˆÂÎ¸: ",other.name);
		// 	}
		// 	else
		// 	{
		// 		Print("‰ÓÒÚÛÔÌ˚ı ˆÂÎÂÈ ÌÂÚ!");
		// 		PrintDebugNpc(PD_MST_CHECK,"...‰ÓÒÚÛÔÌ˚ı ˆÂÎÂÈ ÌÂÚ!");
		// 		return LOOP_END;
		// 	};
		// };
		// if(Npc_IsSummonedByPC(self))
		// {
		// 	PrintSIS("ËÏˇ ˆÂÎË: ",0,other.name);
		// };
	...
	else
	{
		...
		else
		{
			...
			if(Npc_IsSummonedByPC(self))
			{
				Print("œŒ»—  ÕŒ¬Œ… ÷≈À».. ÌÓ‚‡ˇ ˆÂÎ¸ Ì‡È‰ÂÌ‡");
			};
			...
			if(Npc_IsSummonedByPC(self))
			{
				Print("œŒ»—  ÕŒ¬Œ… ÷≈À».. ‰ÓÒÚÛÔÌ˚ı ˆÂÎÂÈ ÌÂÚ!");
			};
