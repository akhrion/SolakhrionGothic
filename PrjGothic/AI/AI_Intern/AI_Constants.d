...
const int BEGGAR_FREQ = 10;//in game minutes
...
const int AIV_DEATHINVFILLED = 24;
...
const int AIV_PARTYMEMBER_FOLLOWDIST = 20;
...
const int AIV_ITEMSCHWEIN = 42;     //при установке в TRUE -> Если на глазах этого НПС взять предмет, то он атакует игрока.
const int AIV_DROPDEADANDKILL = 43;
const int AIV_ABILITY = 48;
const int AIV_ABILITY_DODGE = 1;
const int AIV_FREEMAN = 49;
const int AIV_FREEMAN_WANNATALK = 1;
const int AIV_FREEMAN_ISMINING = 2;
const int AIV_FREEMAN_INITIALIZED = 1 << 2;
const int AIV_ITEROCCUR_DEMON_EXISTENCE = 1 << 3;//регистрирует итерацию состояния вызываемого AI_StartState
const int AIV_ITEROCCUR_ISHEALEDONITERAT = 1 << 4;
const int AIV_FREEMAN_TIMEITERATORSECOND = 1 << 5;//Меняет своё состояние один раз в секунду реального времени. Филд используется в B_Cycle_NPC() -> B_Cycle_NPC вызывается в различных AI_StartState. На основании этого филда, раз в секунду совершаются воздействия на НПС self. Воздействие происходит из B_Cycle_NPC()
const int AIV_FREEMAN_POISONED = 1 << 6;
const int AIV_FREEMAN_HASTE1 = 1 << 7;
...
const int AIV_FREEMAN_TIMEITERATORMINUTE01 = 1 << 12; //Пул переменных предназначенный для минутного цикла НПС
const int AIV_FREEMAN_TIMEITERATORMINUTE02 = 1 << 13;
const int AIV_FREEMAN_TIMEITERATORMINUTE03 = 1 << 14;
...
const int AIV_FREEMAN_TRAINING = 1 << 19;
...
const int Npctype_MystiqueMonster = 10;
...
const int MOBSI_Disassemble = 13;
const int MOBSI_DisassembleEND = 1013;
const int MOBSI_PC_Menu = 15;
const int MOBSI_PC_MenuEND = 1015;
